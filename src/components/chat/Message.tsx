...
console.log("the message",message)
      ...
      buttons:[
        {
          ...
          onClick:async() => {
            try {
              const response =await axios.delete(`http://localhost:3000/api/deleteMessage/${message._id}`)
              if(response.status){
  ...
  return (
    <div className={`chat ${chatClassName}`}>
      ...
      <div className={`chat-bubble text-white ${bubbleBgColor} ${shakeClass} max-w-lg break-words whitespace-pre-wrap`}>
        {message.message ? (
          message.message
        ) : message.imgURL ? (
